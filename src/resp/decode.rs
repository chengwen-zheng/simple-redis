...
fn parse_length(buf: &[u8], prfix: &str) -> Result<(usize, usize), RespError> {
    let end = extract_simple_frame_data(buf, prfix)?;
    let s = String::from_utf8_lossy(&buf[prfix.len()..end]);
    let len = s.parse()?;
    Ok((end, len))
}
